...
public class RestToServicesRoute extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		restConfiguration().component("servlet").enableCORS(true).bindingMode(RestBindingMode.json).dataFormatProperty("prettyPrint", "true");
		...
		.get("search/user/{user}").description("Search for a blog article / user").id("rest-searchbyuser").outTypeList(Blog.class).to("direct:searchByUser")
		.put().id("rest-put-article").description("Put a blog article").type(Blog.class).to("direct:add")
		.delete("{id}").id("rest-deletearticle").description("Delete a blog article").type(Blog.class).to("direct:remove")
		.get("add").id("amq-example").description("AMQ Example").to("amq:add")
		.get("live-hello").id("live-hello").description("Live").route().transform().constant("Hello!");
		 ...
		 * */
		rest("/blog/article").id("rest-options").verb("options").route()
